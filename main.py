...
for fold, split in enumerate(fold_splits):
    ...
    for epoch in range(1, opt.n_epochs + 1):
        ...
        best_score = best_minority_f1 + best_balanced_acc
        
        if (val_results['unique_preds'] > 1 and current_score > best_score) or \
        (val_results['unique_preds'] > 1 and best_minority_f1 == 0):
            ...
            print(f"New best model saved (Minority F1: {best_minority_f1:.3f}, Balanced Acc: {best_balanced_acc:.3f})")
        else:
            patience_counter += 1
